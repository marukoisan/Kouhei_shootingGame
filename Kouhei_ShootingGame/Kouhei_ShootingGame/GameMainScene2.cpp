...
GameMainScene2::GameMainScene2()
{
	...
	enemy2 = new Enemy2 * [10];//エネミーの配列を作成、初期化処理
	for (int i = 0; i < 10; i++)
	{
		enemy2[i] = nullptr;//配列の中にnullptrを入れる
	}
	//enemy[0]番目に入れている
	//enemy[0] = new Enemy(T_Location{ 200,0 });//初期位置を決める場所
	enemy2[0] = new Enemy2(T_Location{ 640,0 }, 1);//初期位置を決める場所
...
void GameMainScene2::Update()
{
	...
	for (enemyCount = 0; enemyCount < 10; enemyCount++)
	{
		...
		enemy2[enemyCount]->Update();//nullptrならUpdate処理を呼び出す処理
	}
			...
			if (enemy2[enemyCount]->HitSphere(bullet[bulletCount]))
			{
				...
				if (enemy2[enemyCount]->HpCheck())
				{
					...
					for (int i = 0; i < 10; i++)
					{
						if (items[i] == nullptr)
						{
							items[i] = new Recovery(enemy2[enemyCount]->GetLocation());//敵が死んだ座標にアイテムを出現させる
							break;//breakで抜ける
						}
					...
					enemy2[enemyCount] = nullptr;//不特定な値を見ないようにするためにnullptrを代入する

					//配列を前に詰める処理
					for (int i = enemyCount; i < 10 - 1; i++)
					{
						if (enemy2[i + 1] == nullptr)
						{
							break;
						}
						enemy2[i] = enemy2[i + 1];
	...
	for (enemyCount = 0; enemyCount < 10; enemyCount++)
	{
		...
		BulletsBase** bullet = enemy2[enemyCount]->GetBullets();//エネミーが配列なのでエネミー一つ一つのデータを持ってくるため
...
void GameMainScene2::Draw() const
{
	...
	for (int enemyCount = 0; enemyCount < 10; enemyCount++)
	{
		...
		enemy2[enemyCount]->Draw();//nullptrならDraw処理を呼び出す処理
	}
...
int GameMainScene2::EnemyDown()//カウントされた数字をもらう
{
	enemyDown++;
